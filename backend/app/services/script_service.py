...
def _to_script_dict(script: ScriptRequest) -> dict:
    return {
        ...
        "script_sequence": script.script_sequence,
        "server_sequence": script.server_sequence,
...
async def process_validation(script_id: str) -> None:
    ...
    if script.bundle_id:
        from app.services.bundle_service import sync_bundle_status

        await sync_bundle_status(script.bundle_id)
...
RESUBMITTABLE_SCRIPT_STATUSES = {
    ScriptStatus.AUTO_REJECTED,
    ScriptStatus.MANUALLY_REJECTED,
    ScriptStatus.EXECUTION_FAILED,
}


def can_resubmit_script(user: User, script: ScriptRequest) -> bool:
    if script.bundle_id is not None:
        return False
    if script.status not in RESUBMITTABLE_SCRIPT_STATUSES:
        return False
...
async def resubmit_script(script_id: str, user: User, tsql_content: str) -> ScriptRequest:
    script = await ScriptRequest.get(script_id)
    if not script:
        raise ValueError("Script não encontrado")
    if not can_resubmit_script(user, script):
        raise PermissionError("Não é possível reenviar esta solicitação")

    script.tsql_content = tsql_content
    script.content_hash = compute_content_hash(tsql_content)
    script.status = ScriptStatus.VALIDATING
    script.validation_result = []
    script.approval = None
    script.execution_result = None
    script.validated_at = None
    script.executed_at = None
    script.updated_at = datetime.utcnow()
    await script.save()

    asyncio.create_task(process_validation(script_id))
    return script
